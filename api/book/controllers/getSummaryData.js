...
const getSummaryData = async (ctx) => {
    ...
    const query6 = `
        ...
        `
    const query7 = `
        SELECT COUNT(*) FROM borrow_books bb INNER JOIN borrows br
            ON bb.borrow = br.id 
        WHERE DATE(br.date) = CURDATE()
    `
    const query8 = `
        SELECT COUNT(*) FROM borrow_books  bb
        WHERE DATE(bb.returnDate) = CURDATE()
    `
    ...
    const countBorrowToday = await strapi.connections.default.raw(query7)
    const countReturnToday = await strapi.connections.default.raw(query8)
    ...
    ctx.send({
        ...
        borrowToday: countBorrowToday[0][0].total,
        returnToday: countReturnToday[0][0].total
    });
}
